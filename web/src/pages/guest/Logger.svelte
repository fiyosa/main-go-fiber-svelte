<script lang="ts">
  ...
  function totalPages(): number {
    if (!detailQuery.data) return 1
    return Math.ceil(detailQuery.data.total / detailQuery.data.limit) || 1
  }
    ...
    {#if selectedFile}
      <div class="flex items-center gap-2 p-3 border-b border-gray-800 bg-gray-950 flex-wrap">
        ...
        <div class="flex items-center gap-1 flex-wrap">
          <button
            class="text-xs px-3 py-1.5 rounded font-semibold uppercase transition bg-blue-700 text-white {(selectedLevels.includes('info') || selectedLevels.length === 0) ? 'ring-2 ring-white/40' : 'ring-0 opacity-60'}"
            onclick={() => toggleLevel('info')}
          >info</button>
          <button
            class="text-xs px-3 py-1.5 rounded font-semibold uppercase transition bg-red-700 text-white {(selectedLevels.includes('error') || selectedLevels.length === 0) ? 'ring-2 ring-white/40' : 'ring-0 opacity-60'}"
      ...
      <div class="flex-1 overflow-y-auto">
        ...
        {:else if (detailQuery.data?.logs ?? []).length > 0}
          {#each detailQuery.data?.logs ?? [] as entry, i}
            <div class="border-b border-gray-800">
              <button
                class="w-full flex items-start gap-2 px-4 py-2.5 text-left hover:bg-gray-800/30 transition"
                onclick={() => toggleExpand(i)}
              >
                <span class="shrink-0 mt-0.5 text-xs text-gray-700">{expandedIndex === i ? "▼" : "▶"}</span>
                <span
                  class="shrink-0 text-xs font-mono px-2 py-0.5 rounded font-semibold uppercase {levelBadge(entry.level)}"
                >{entry.level}</span>
      ...
      {#if detailQuery.data && totalPages() > 1}
        <div class="flex items-center justify-between px-4 py-2 border-t border-gray-800 bg-gray-950">
          <span class="text-xs text-gray-500">{detailQuery.data.total} entries</span>
